...
/**
 * 1. Ініціалізація базової карти Leaflet
 */
function initMap() {
  const mapContainer = document.getElementById('map');
  if (!mapContainer) return;

  // Центруємо карту на Черкаській області
  appState.mapInstance = L.map('map', {
    zoomControl: false,
    attributionControl: false
  }).setView([49.4444, 32.0598], 8);
...
function processFinancialData(rawData) {
  ...
  const processedHromadas = filteredRows.map((row, index) => {
    ...
    const factPrevYear = parseNum(row['Аналогічний період минулого року'] || row['Факт минулого року']);
...
/**
 * Колірна шкала (Choropleth) за % виконання плану
 */
function getColor(percent) {
  if (percent === null || percent === undefined || isNaN(percent) || percent === 0) {
    return '#cbd5e1'; // Немає даних (світло-сірий)
  }
  if (percent >= 100) {
    return '#10b981'; // 100% і більше (Виконання - смарагдово-зелений)
  } else if (percent >= 95) {
    return '#f59e0b'; // 95% – 100% (Зона ризику - теплий жовтий)
  } else {
    return '#e11d48'; // Менше 95% (Недовиконання - карміново-червоний)
  }
}
...
function updateBlockA() {
  ...
  if (appState.oblastData) {
    ...
    rateEl.style.color = getColor(appState.oblastData.execRate);
  } else {
  ...
  appState.mapLayer = L.geoJSON(currentGeoJson, {
    style: (feature) => {
      ...
      return {
        ...
        opacity: 1,
        color: isSelected ? '#0057b8' : '#334155',
    ...
    onEachFeature: (feature, layer) => {
      ...
      const tooltipContent = `
        <div class="custom-tooltip-content">
          ...
          <p><strong>Виконання плану:</strong> <span style="font-weight:700; color:${getColor(execRate)}">${formatPercent(execRate)}</span></p>
...
function selectCommunity(id) {
  ...
  // Зумування карти до обраного полігону
...
function updateMiniChart() {
  ...
  appState.barChart.data = {
    ...
    datasets: [
      {
        label: 'План звітний період',
        data: displayData.map(r => (r.planPeriodRaw || 0) / 1000),
        backgroundColor: 'rgba(0, 87, 184, 0.85)', // Нейтрально-синій
